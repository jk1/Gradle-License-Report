...
licenseReport {
    ...
    importers = arrayOf<DependencyDataImporter>(
        XmlReportImporter("Front End", layout.projectDirectory.file("externalDependencies.xml").asFile)
    )

    allowedLicensesFile = layout.projectDirectory.file("no-allowed-licenses.json")
}
